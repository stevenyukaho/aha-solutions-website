...
import { whoThisIsForBullets } from '@/lib/data';
    ...
    <div className="card" style={{ padding: '44px 36px', marginBottom: 20 }}>
      <div className="who-for-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1.1fr)', gap: 48, alignItems: 'center' }}>
        <div style={{ minWidth: 0 }}>
          <div className="mono" style={{ fontSize: 10.5, letterSpacing: 1.6, color: 'var(--accent)', marginBottom: 14 }}>WHO THIS IS FOR</div>
          <h2 style={{ fontSize: 'clamp(24px, 2.8vw, 34px)', lineHeight: 1.15, marginBottom: 18 }}>This is for you if:</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {whoThisIsForBullets.map((b) => (
          ...
          <p style={{ fontSize: 15, color: 'var(--text)', marginTop: 26, marginBottom: 0, lineHeight: 1.5 }}>
            If that sounds familiar, <span style={{ color: 'var(--accent)' }}>you don&apos;t need more tools.</span> You need systems.
          </p>
        </div>
        <div style={{ padding: '36px 32px', borderRadius: 14, background: 'var(--bg-3)', border: '1px solid var(--line)', position: 'relative' }}>
          ...
          <p style={{ fontSize: 19, color: 'var(--text)', lineHeight: 1.45, margin: 0, fontFamily: 'Instrument Serif, serif', fontStyle: 'italic' }}>
            I&apos;ve been on both sides — as the operator and the CEO. I build systems I&apos;d use in my own businesses.
          </p>
          <div className="mono" style={{ fontSize: 10.5, color: 'var(--text-3)', letterSpacing: 1.4, marginTop: 18 }}>— STEVEN YU</div>
        </div>
      </div>
    </div>
  );
